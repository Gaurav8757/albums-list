      ...
      <div className="grid grid-cols-3 sm:grid-cols-4  lg:grid-cols-4 xl:grid-cols-5 gap-8 justify-center ">
        {Array.from(new Set(albums.map(album => album.userId))).map(userId => (
          <Link
            key={userId}
            to={`/user/${userId}`}
            className="border group hover:text-2xl transition-all duration-500 h-32 sm:h-32 md:h-40 lg:h-60 xl:h-72 w-32 sm:w-32 md:w-40 lg:w-60 xl:w-72 flex justify-center  cursor-pointer aspect-w-1 aspect-h-1  border-sky-200 shadow-[0_0_2px_#fff,inset_0_0_2px_#fff,0_0_5px_#08f,0_0_15px_#08f,0_0_30px_#08f]">
